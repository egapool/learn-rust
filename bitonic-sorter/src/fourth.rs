...
const PARALELLIZE_THRESHOLD: usize = 4096;
    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        ...
        if mid_point >= PARALELLIZE_THRESHOLD {
            rayon::join(
    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        ...
        let mid_point = x.len() / 2;
        let (first, secone) = x.split_at_mut(mid_point);
        if mid_point >= PARALELLIZE_THRESHOLD {
            rayon::join(
                || sub_sort(first, forward, comparator),
                || sub_sort(secone, forward, comparator),
            );
        } else {
...
mod tests {
    ...
    use crate::{
        utils::{is_sort_ascending, is_sort_descending, new_u32_vec},
        SortOrder::*,
    };
    ...
    #[test]
    fn 長さが2のべき乗になっていないとエラー() {
        let mut x = vec![10, 30, 1];
    ...
    fn sort_students_by_name_ascending() {
        ...
        assert_eq!(
            sort_by(&mut x, &|a, b| a // TODO: クロージャの &は何？
                .last_name
                .cmp(&b.last_name) // TODO: aは&なしでbは借用。
                .then_with(|| a.first_name.cmp(&b.first_name))), // TODO: || はどういう意味？
            Ok(())
        );
